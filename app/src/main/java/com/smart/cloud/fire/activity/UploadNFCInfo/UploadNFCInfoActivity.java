...
import com.smart.cloud.fire.adapter.NFCCheakItemAdpter;
...
public class UploadNFCInfoActivity extends Activity {
    ...
    @Bind(R.id.nfc_cheak_items_recycler)
    RecyclerView nfc_cheak_items_recycler;
    ...
    private List<NFCCheakItem> cheakItems;
    private NFCCheakItemAdpter mAdapter;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case 0:
                    ...
                    cheakItems.removeAll(cheakItems);
                    mAdapter = new NFCCheakItemAdpter(cheakItems);
                    //设置Adapter
                    nfc_cheak_items_recycler.setAdapter(mAdapter);
                    break;
                case 1:
    ...
    private void getCheakItems(String devType) {
        VolleyHelper helper=VolleyHelper.getInstance(mContext);
        RequestQueue mQueue = helper.getRequestQueue();
        String url= ConstantValues.SERVER_IP_NEW+"getNFCCheakItems?devType="+devType;
                ...
                new Response.Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject response) {
                        try {
                            ...
                            if(errorCode==0){
                                cheakItems=new ArrayList<>();
                                JSONArray jsonArray=response.getJSONArray("items");
                                for(int i=0;i<jsonArray.length();i++){
                                    JSONObject b=jsonArray.getJSONObject(i);
                                    NFCCheakItem item=new NFCCheakItem();
                                    item.setItemname(b.getString("itemName"));
                                    item.setId(b.getInt("id"));
                                    item.setState(true);
                                    cheakItems.add(item);
                                }
                                if(cheakItems.size()>0){
                                    mLayoutManager = new LinearLayoutManager(mContext);
                                    //设置布局管理器
                                    nfc_cheak_items_recycler.setLayoutManager(mLayoutManager);
                                    //绑定数据
                                    mAdapter = new NFCCheakItemAdpter(cheakItems);
                                    mAdapter.setOnItemChangeListener(new NFCCheakItemAdpter.OnItemChangeListener() {
                                        @Override
                                        public void onItemChangeClick(int position, boolean state) {
                                            cheakItems.get(position).setState(state);
                                        }
                                    });
                                    //设置Item增加、移除动画
                                    nfc_cheak_items_recycler.setItemAnimator(new DefaultItemAnimator());
                                    //设置Adapter
                                    nfc_cheak_items_recycler.setAdapter(mAdapter);
                                }
                            }else{
            ...
            public void onClick(View v) {
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        for(int i=0;i<cheakItems.size();i++){
                            JSONObject obj= new JSONObject();
                            NFCCheakItem item=cheakItems.get(i);
                            try {
                                obj.put("id",item.getId());
                                obj.put("item",item.getItemname());
                                obj.put("state",item.isState());
                                jsonArray.put(obj);
                            } catch (JSONException e) {
                                e.printStackTrace();
                            }
                        }
                        final String states=jsonArray.toString();
...
//                        RequestQueue mQueue = Volley.newRequestQueue(mContext);
                        ...
                        if(isHavePhoto&&isSuccess){
                            ...
                            photo1=areaId+"\\"+uploadTime+imageFilePath.substring(imageFilePath.lastIndexOf("."));
...
//                                    +"&photo1="+"&items="+states;
                        ...
                        url= ConstantValues.SERVER_IP_NEW+"addNFCRecord";
                        StringRequest mRequest = new StringRequest(Request.Method.POST,
                            ...
                            protected Map<String, String> getParams() throws AuthFailureError {
                                ...
                                map.put("uid", uid_name.getText().toString());
                                map.put("longitude", lon);
                                map.put("latitude", lat);
                                map.put("devicestate", deviceState);
                                map.put("memo", memo_name.getText().toString());
                                map.put("photo1", photo1);
                                map.put("items",states);
                                return map;
                            }
...
//            }
    ...
    private void setNoteBody(String body) {
        try {
            ...
            getCheakItems(jsonObject.getString("deviceTypeId"));//！！！！！！！！！！！！！！！！！
