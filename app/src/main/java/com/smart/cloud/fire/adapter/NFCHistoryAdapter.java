...
//                    .load("http://139.159.209.212:51091/nfcimages/2017/1506499353470.jpg").thumbnail((float)0.0001)
            ...
            ((ItemViewHolder) holder).items_line.removeAllViews();
            if(normalSmoke.getItems()!=null||normalSmoke.getItems().length()>0){
                try {
                    JSONArray jsonArray=new JSONArray(normalSmoke.getItems());
                    for(int i=0;i<jsonArray.length();i++){
                        JSONObject obj=jsonArray.getJSONObject(i);
                        addViewItem(((ItemViewHolder) holder).items_line,obj.getString("item")+":",(obj.getBoolean("state")?"合格":"不合格"));
                    }
    ...
    public static class ItemViewHolder extends RecyclerView.ViewHolder {
        ...
        @Bind(R.id.items_line)
        LinearLayout items_line;
    ...
    //添加ViewItem
    private void addViewItem(LinearLayout view,String n,String v) {
            View itemView = View.inflate(mContext, R.layout.nfc_cheak_items, null);
            TextView name = (TextView) itemView.findViewById(R.id.item_name);
            TextView value = (TextView) itemView.findViewById(R.id.item_value);
            name.setText(n);
            value.setText(v);
